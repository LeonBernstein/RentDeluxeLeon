...
import { VehicleModelI } from 'src/app/ts/interfaces/model.interfaces';
...
export class CarModelsComponent implements OnInit, OnDestroy {
	...
	public get columns(): string[] {
		return ['index', 'vehicleModelId', 'isActive', 'name', 'manufacturer.name', 'vehicleType.name', 'edit', 'toggleActive']
	}
	
	public fastSearch: string = ''
	public showOnlyActive: boolean = true
	
	private _entities: VehicleModelI[]
	private _changeSub: Subscription
	
	private _dataSource: MatTableDataSource<VehicleModelI>
	public get dataSource(): MatTableDataSource<VehicleModelI> {
	...
	private _setSubscribers(): void {
		this._changeSub = this._ChangesService.carModelChange
			.subscribe(data => {
				let isNew = true
				this._entities.forEach(x => {
					if (x.vehicleModelId === data.vehicleModelId) {
				...
				if (isNew) {
					const newEntity = ModelsFactory.createVehicleModelModel()
		...
		this._Http.VehicleModelsApi
			...
			.subscribe(r => {
				if (r && Array.isArray(r)) {
					this._entities = new Array<VehicleModelI>()
					r.forEach(x => {
						let entity = ModelsFactory.createVehicleModelModel()
	...
	public deleteUser(id: number): void {
		this._Http.VehicleModelsApi
			...
			.subscribe(r => {
				if (r) {
					this._entities.forEach(x => {
						if (x.vehicleModelId === id) Object.assign(x, r)
					})
					this.applyFilters()
	...
	public applyFilters(): void {
		const filters: FilterI[] = [
			...
			{
				id: 'fastSearch',
				columns: ['vehicleModelId', 'name', 'manufacturer.name', 'vehicleType.name'],
	...
	private _setFilterPredicate(): void {
		(<any>this._dataSource).filterPredicate = (data: VehicleModelI, filtersJson: FilterI[]): boolean => {
